...
(function(window) {
  ...
  var draw = function() {
    ...
    // Aspect zoom
    var width = cw;
    var height = Math.ceil(images.phobos.height / (images.phobos.width / cw));
    if (height < canvas.height) {
      width = Math.ceil(images.phobos.width / (images.phobos.height / ch));
      height = ch;
    }
    ...
    var scale = images.phobos.width / width;

    // Moon
    ctx.globalAlpha = 1;
    ctx.fillStyle = "#111";
    ctx.fillRect(0, 0, canvas.width, canvas.height);
    ctx.drawImage(images.phobos, x, y, width, height);
